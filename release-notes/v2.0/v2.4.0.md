...
## Deployment Notes
- From the repository root (`/project`), run `npx prisma db push --schema=prisma/schema.postgres.prisma --accept-data-loss` on Render (or apply equivalent SQL) to add the new columns and the `StrengthProfile` table. Running the command inside `lifecraft-bot/` will reference an outdated schema and drop required tables.
