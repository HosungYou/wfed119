...
### 2. **Discover Vision API Promotion (discover 2 → discover)**
...
- `GET /api/health` → returns `status: healthy` or `degraded` with Supabase details
- `GET /api/discover/vision/session` → creates/loads `vision_statements` for the user
- `POST /api/enneagram/answer` → persists stage responses to `enneagram_sessions`
- `POST /api/enneagram/score` → returns `primaryType`, `wingEstimate`, `instinct`
- `POST /api/chat/stream` → inserts `conversation_messages` + `strength_profiles`
- `GET /api/admin/database/stats` → requires `SUPABASE_SERVICE_ROLE_KEY` + SUPER_ADMIN
- `GET /login` → Supabase OAuth login flow
