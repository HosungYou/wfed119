...
**Old Flow**: Q1-Q6 → Patterns → Themes → Results

**New Flow**: Q1-Q6 → Findings → Follow-up → Results
...
Added breadcrumb navigation and session reset functionality:
...
<div className="flex items-center gap-2 text-sm">
  ...
  <button onClick={() => router.push('/discover')}>
    <LayoutDashboard /> {language === 'en' ? 'Dashboard' : '대시보드'}
  </button>
  <ChevronRight />
  <span>Life Themes</span>
</div>

// Session Reset Button (uses SessionResetButton component)
<SessionResetButton
...
Features:
- Home → Dashboard → Module breadcrumb navigation
- i18n support (Korean/English)
- Session reset with confirmation dialog
- Page refresh on reset completion

---

### 6. **Image Export Functionality**

**File**: `src/app/discover/life-themes/results/page.tsx`

Added ability to save results as PNG image:
...
export const PART_NAMES: Record<ModulePart, { en: string; ko: string }> = {
  ...
  'mission-roles': { en: 'Mission & Roles', ko: '사명 & 역할' },
...
```bash
npm run build
# ✅ Compiled with warnings in 25.3s
...
| `src/app/discover/life-themes/page.tsx` | Breadcrumb navigation, SessionResetButton, i18n, teal/purple theme |
| `src/app/discover/life-themes/results/page.tsx` | Image export, navigation header, i18n, findings/followup display |
...
| `src/components/modules/SessionResetButton.tsx` | Reusable session reset component |
...
**Algorithmic Achievement**: Flow restructure from Patterns→Themes to Findings→Follow-up, PDF worksheet alignment, image export

**Next Release**: v3.3.2 - VS Diverge Design enhancements + Life Roles module
...
4. **Verify Module Order**:
   - Dashboard should show: Enneagram → Life Themes → Values → Mission → Life Roles → Vision → SWOT → Career Options → Goals → ERRC

---

**Upgrade Priority**: 🟡 **Medium** - Contains major UI/flow changes
