...
import { Loader2, ArrowRight, ArrowLeft, GripVertical, Table2, BarChart3, StickyNote } from 'lucide-react';
...
const SHARPEN_SAW_ROLE: LifeRole = {
  id: 'sharpen-the-saw',
  entity: 'Self',
...
export default function LifeRolesStep3() {
  ...
  async function loadData() {
    try {
      ...
      console.error('[Life Roles Step 3] Error:', error);
      setLoading(false);
    }
  }
  ...
  function handleDrop(e: DragEvent, slotIndex: number) {
    ...
    const existingIndex = newSlots.findIndex((_, i) => i === slotIndex);

    // Check if slot already occupied — replace it
    const currentSlotEntry = newSlots[slotIndex];
    if (currentSlotEntry) {
      newSlots[slotIndex] = {
        ...currentSlotEntry,
        roleId: draggedRole.id,
        roleName: draggedRole.role === 'Sharpen the Saw'
          ? 'Sharpen the Saw'
          : `${draggedRole.entity}: ${draggedRole.role}`,
      };
    } else {
  ...
  async function handleSave() {
    ...
    try {
      ...
      console.error('[Life Roles Step 3] Save error:', error);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleNext() {
    const placedCount = rainbowData.slots.filter(s => s.roleId).length;
    if (placedCount < 3) {
      alert(language === 'ko'
  ...
  const placedCount = rainbowData.slots.filter(s => s.roleId).length;
  const allDraggableRoles: LifeRole[] = [SHARPEN_SAW_ROLE, ...lifeRoles];
          ...
          <div className="lg:col-span-1">
            <ModuleCard padding="normal">
              ...
              <div className="space-y-2">
                {/* Sharpen the Saw special card */}
                <div
                  draggable
                  onDragStart={(e) => handleDragStart(e, SHARPEN_SAW_ROLE)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-purple-800">Sharpen the Saw</p>
                    <p className="text-xs text-purple-600">Self</p>
                  </div>
                </div>
              ...
              {viewMode === 'arc' ? (
                <div>
                        ...
                        >
                          {slot ? (
                            ...
                            <>
                              ...
                              {/* Age range editors (appear below on hover/always visible) */}
                              <div className="absolute -bottom-7 left-0 right-0 flex items-center gap-1 px-2 opacity-0 group-hover:opacity-100 pointer-events-none">
                              </div>
                            </>
                          ) : (
                    ...
                    {Array.from({ length: 8 }, (_, index) => {
                      ...
                      return (
                        <div key={index} className="flex items-center gap-3 p-2 rounded-lg bg-gray-50">
                          ...
                          />
                          <span className="text-xs font-medium text-gray-700 w-32 truncate">{slot.roleName}</span>
                  ...
                  <p className="text-center text-xs text-gray-400 mt-4">
                    Life Roles Rainbow — {placedCount}/8 {language === 'ko' ? '배치됨' : 'placed'}
                  </p>
                </div>
              ) : (
                /* Table View */
                    ...
                    <table className="w-full text-sm border-collapse">
                      ...
                      <tbody>
                        {Array.from({ length: 8 }, (_, index) => {
                          ...
                          const color = RAINBOW_COLORS[index];
                          return (
                            <tr key={index} className={slot ? 'bg-white' : 'bg-gray-50'}>
                              ...
                              <td className="border border-gray-200 px-3 py-2">
                                {slot ? (
                                  <span className="font-medium text-gray-900">{slot.roleName}</span>
                                ) : (
        ...
        <ModuleCard padding="normal" className={placedCount >= 3 ? 'bg-green-50 border-green-200' : 'bg-gray-50'}>
          <div className="flex items-center justify-between">
            <p className={placedCount >= 3 ? 'text-green-700' : 'text-gray-600'}>
              {language === 'ko'
                ? `${placedCount}/8 역할 배치 완성 (최소 3개 필요)`
                : `${placedCount}/8 roles placed (minimum 3 required)`}
            </p>
